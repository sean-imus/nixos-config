...
- **Personal Quickshell shell — step 2: single-process shell (2026-09-17)**: `qs-shell` now owns everything in one process: the clock/workspaces bar (parity widgets: workspaces via a `niri msg -j event-stream` singleton `services/Niri.qml`, battery/vol/mic/power-profile right side, click/scroll actions), the notification server (`services/Notifs.qml`, plain popups top-right, DND + clear via `Mod+Shift+D`/`Mod+Shift+N`, no history persistence — in-memory only), a Quickshell `PolkitAgent` auth dialog (`modules/polkit/PolkitDialog.qml`; polkit's own `PolkitAgentSession`/helper and the stock `polkit-1` PAM, so no PAM service needed), and an OSD pill (volume/mic via Pipewire, brightness via a 500 ms sysfs poll — inotify does not work on sysfs, power profile via PowerProfiles) plus `Mod+P` cycles profiles. Retired: waybar (`features/bar.nix` deleted), caelestia-notifs (`features/notifications/` deleted; own notification server now owns `org.freedesktop.Notifications`), soteria (`features/soteria.nix` deleted, `security.soteria.enable` removed, `polkit-soteria.service` line dropped from `rebuild.nix`; the quickshell polkit agent replaces it — if the shell dies, elevation prompts wedge until it's restarted, same failure mode soteria had). Quickshell API gotchas learned: services singletons need `import Quickshell` even when importing e.g. `Quickshell.Services.Pipewire`; `import Quickshell.Wayland` required for `WlrLayershell`/`WlrKeyboardFocus`; `property list<var>` deep-copies JS objects so `indexOf(entry)` breaks reference identity — use `property var`; niri-flake merges sibling `_children` with the same name, so repeated `layer-rule` blocks need one `_children` parent each; initial service state loads (Pipewire connect, sysfs poll, PowerProfiles init) fire property changes, so the OSD arms 1 s after startup. Verified live: config loads clean, notification popup shows/expires, `notifs toggleDnd|clear` IPC, OSD fires on brightness/volume/profile changes, polkit dialog appears on `run0` and cancels with the requestor. After `nh os switch` + relogin, niri spawns `quickshell -c qs-shell -n` at startup. Live right now: qs-shell runs from the store path as the sole shell process; waybar/soteria stopped.

- **Personal Quickshell shell — step 1: clock bar (2026-09-16)**: a from-scratch Quickshell config (deliberately not caelestia-derived) lives in `modules/features/quickshell/` as HM config `qs-shell` (`programs.quickshell.configs.qs-shell`). `default.nix` copies `qml/` into a derivation and generates `config/Theme.qml` from `lib/theme.nix` (same pattern as notifications). `shell.qml` spawns a `Bar.qml` per screen: bottom 18px transparent bar, waybar geometry, clock left, namespace `qs-shell-bar`, exclusive zone 18. Not spawned at startup yet — waybar stays the active bar; launch `quickshell -c qs-shell` after a switch (running both reserves 36px at the bottom). Dev loop without switching: `nix build --impure --expr 'let f = builtins.getFlake "/home/sean/nixos-config"; in f.nixosConfigurations.notebook.config.home-manager.users.sean.xdg.configFile."quickshell/qs-shell".source'`, then `quickshell --path <out>`. Smoke test passed (config loads, `niri msg layers` shows two `qs-shell-bar` surfaces). Next: niri event-stream service for workspaces/windows, then battery/volume widgets, then retire waybar; caelestia-notifs keeps notifications until reimplemented here. Flake note: new files must be `git add`ed or they are invisible to Nix.
