...
- Notifications: mako with everforest styling (`modules/features/notifications.nix`), running as a systemd user service plus D-Bus activation. `Mod+U` runs `nh os switch` in the background (notification on start; activation elevates via run0 -> soteria prompt; notification replaced with success/failure). Log: `~/.cache/nh-os-switch.log`.
