...
- Notifications: mako with everforest styling (`modules/features/notifications.nix`), running as a systemd user service plus D-Bus activation. `Mod+U` builds with `nh os build` in the background, then applies the result with a single run0 elevation (one soteria prompt), replacing the notification with success/failure. Log: `~/.cache/nh-os-switch.log`.
